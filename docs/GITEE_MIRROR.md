...
4. 查看工作流日志：符合 Gitee 体积上限的安装包上传后必须通过免登录完整下载及 SHA-256 校验。所有待镜像文件成功后才写入更新清单；超限文件不会下载或上传，只保留原 GitHub 地址，发布页和清单会明确区分。日志同时报告实际 `Range` 支持情况。
...
- 实测本仓库附件上限为 100 MB。脚本使用保守的 100,000,000 字节作为上传前阈值；超限文件仅保留 GitHub 下载，不阻止其他平台发布。所有文件均超限时，在下载及远程写入前明确报错。其他网络、权限、校验或容量错误仍会终止发布。
- 当前 `v2.3.1+4029` 通用 APK 为 111,829,288 字节，Gitee 已明确拒绝上传。当前可发布 Windows 国内包，Android 暂用 GitHub 原包。Android 国内直装需另行发布并验证小于上限的对应架构 APK，或使用容量足够的下载服务。当前 APK 是 Shorebird 基线，不要用目录中旧的普通 Flutter 分架构产物替换，也不要将重新构建的不同内容伪装成原版同名安装包。
