...
沿用 `2.3.1+4029` 的独立热更新基线，仅移入上述 Dart 修复。两端构建和兼容检查成功，未强制放行原生或资源差异。

| 平台 | 补丁 | 服务端 ID | 通道 | 架构 |
| --- | --- | --- | --- | --- |
| Windows | #5 | 666944 | stable | x86_64 |
| Android | #6 | 666945 | stable | arm、aarch64、x86_64 |

`shorebird patches info` 回读确认上述编号、平台、stable 和未回滚状态。主线源码提交 `c23055c`；旧安装基线源码提交 Android `832df72`、Windows `bc7e1d2`，两者源码树相同。主线及旧基线源码均已推送 GitHub，主线 Gitee 同步成功。

应用内补丁说明对应 GitHub prerelease `v2.3.1+4029-patch.5` / `v2.3.1+4029-patch.6`，没有替换完整安装包的 latest 发布。用户需再次检查更新，补丁下载就绪后完全退出并重启，确认 Android #6 / Windows #5；安装版本号仍为 `2.3.1+4029`。
