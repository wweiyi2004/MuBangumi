...
function adminContent(){const r=current();
  if(tab==='members')return `<div class="card"><h2>入场名单</h2><p class="muted">仅显示当前轮次的提交状态，不展示姓名与分数、评论的对应关系。</p><div class="member-grid">${event.members.map(m=>`<div class="member row between"><strong>${esc(m.name)}</strong><span class="badge">${m.submitted?'已评分':'未评分'}</span></div>`).join('')}</div></div>`;
  ...
  if(tab==='results'){return `<div class="card"><div class="row between"><h2>评分结果</h2><button data-action="project">大屏展示</button></div><p class="hint">管理端可查看所有统计；参与者仅能查看已公布轮次。</p>${event.rounds.map(v=>`<section class="card">${hero(v)}${metrics(v)}${bars(v)}<button class="soft" data-action="publish" data-round="${v.id}" data-value="${!v.published}">${v.published?'撤回公布':'公布这一轮结果'}</button></section>`).join('')}</div>`;}
...
function renderParticipantSheet(){
  ...
  let content='';
  if(kind==='playlist')content=event.rounds.map(v=>`<div class="list-row"><div class="grow"><strong>${esc(v.subject.title)}</strong><small> · ${status(v)}</small></div><button class="link" data-action="review-round" data-id="${v.id}">查看</button></div>`).join('');
  else if(!r)content='<p class="muted">等待主持人开始</p>';
  else content=`<h3>${esc(r.subject.title)}</h3>${selectedRound?'<button class="link" data-action="review-current">返回当前轮次</button>':''}`+(kind==='comments'?`<p class="hint">${r.publicComments?'大家的匿名短评':'短评墙尚未开放，仅展示你自己的评论'}</p>${comments(r)}`:r.stats?metrics(r)+bars(r):'<div class="empty">结果暂未公布，个人分数仅自己可见。</div>');
  dialog.innerHTML=`<div class="row between"><h2>番键会现场</h2><button class="link" data-action="close-modal">关闭</button></div><nav class="tabs">${[['comments','评论墙'],['results','统计'],['playlist','番单']].map(([id,label])=>`<button class="${kind===id?'active':''}" data-action="participant-sheet" data-id="${id}">${label}</button>`).join('')}</nav><div id="participant-sheet-content">${content}</div>`;
...
document.addEventListener('click',async e=>{const b=e.target.closest('[data-action]');if(!b||b.disabled)return;const a=b.dataset.action,r=b.dataset.round;try{
  ...
  else if(a==='export'){modal('<h2>导出活动记录</h2><p>导出汇总评分和匿名短评，不包含姓名与评分对应表。</p><div class="actions"><button data-action="download" data-format="scores">评分 CSV</button><button data-action="download" data-format="comments">评论 CSV</button><button data-action="download" data-format="json">活动 JSON</button></div>');}
  else if(a==='download'){const res=await fetch('/api/export?event='+event.id+'&format='+b.dataset.format,{headers:{Authorization:'Bearer '+token}});if(!res.ok)throw Error('导出失败，请重新登录管理端');const url=URL.createObjectURL(await res.blob());const el=document.createElement('a');el.href=url;el.download='番键会-'+b.dataset.format+(b.dataset.format==='json'?'.json':'.csv');el.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
